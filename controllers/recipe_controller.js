...
const breakfast_index = (req, res) => {
  Recipes.find({category: "Breakfast"}, (error, allRecipes) => {
    error? console.log(error)
    :
    res.render('recipes/b_index.ejs', {
      recipe: allRecipes,
      currentUser: req.session.currentUser
    })    
  })  
...
const lunch_index = (req, res) => {
  Recipes.find({category: "Lunch"}, (error, allRecipes) => {
    error? console.log(error)
    :
    res.render('recipes/l_index.ejs', {
      recipe: allRecipes,
      currentUser: req.session.currentUser
    })    
  })  
...
const dinner_index = (req, res) => {
  Recipes.find({category: "Dinner"}, (error, allRecipes) => {
    error? console.log(error)
    :
    res.render('recipes/d_index.ejs', {
      recipe: allRecipes,
      currentUser: req.session.currentUser
    })    
  })  
  ...
  Recipes.create([
    {
      ...
      picture: "https://www.hiddenvalley.com/wp-content/uploads/2021/04/turkey-club-ranch-pasta-salad-RDP.jpg",
    ...
    },
    {
      ...
      "Add eggs, 2 tspns extract", "Mix the waffle batter", 
      "Make the waffles in a deep pocket waffles iron", 
      "Serve"],
      prepTime: 10,
      cookTime: 20,
      servings: 6
